...
/**
 * @author xuj
 * @since 2017-05-05-11:07
 */
public class MyHttpsServer extends MyHttpsBase {
    ...
    public MyHttpsServer(String type) throws CertificateException {
        super(type);
    }
    ...
    private void doSocketTransport() {
        try {
            System.out.println("--------------------------------------------------------");
    ...
    public byte[] readBytes(int length) throws Exception {
        byte[] cipherText = SocketUtils.readBytes(in, length);
        System.out.println("读取未解密消息:" + byte2hex(cipherText));
        return codec.decrypt(cipherText, key);
    }

    public void writeBytes(byte[] data) throws Exception {
        byte[] encrypt = codec.encrypt(data, key);
        System.out.println("写入加密后消息:" + byte2hex(encrypt));
        SocketUtils.writeBytes(out, encrypt, encrypt.length);
    }

    private void doHttpsShakeHands(Socket s) throws Exception {
        ...
        //第二步，发送服务器证书到客户端
        byte[] certificateBytes = manager.readCertificates();
        privateKey = manager.readPrivateKeys();
        ...
        byte[] desKey = codec.initSecretKey(secureRandom);
        key = codec.toKey(desKey);

        byte serverMessage[] = codec.encrypt(randomMessage.getBytes(), key);
