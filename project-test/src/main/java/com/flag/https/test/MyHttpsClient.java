 ...
 */
public class MyHttpsClient extends MyHttpsBase {
    ...
    public MyHttpsClient(String type) throws CertificateException {
        super(type);
    }
    ...
    public static void main(String args[]) throws Exception {
        ...
        System.out.println("------------------------------------------------------------------");
        String name = "duck";
        client.writeBytes(name.getBytes());

        int len = in.readInt();
        byte[] msg = client.readBytes(len);
    ...
    private void shakeHands() throws Exception {
        ...
        byte[] desKey = codec.initSecretKey(new SecureRandom(seedBytes));
        key = codec.toKey(desKey);

        byte[] decrpytedServerMsg = codec.decrypt(serverMessage, key);
    ...
    public byte[] readBytes(int length) throws Exception {
        ...
        return codec.decrypt(undecrpty, key);
    }

    public void writeBytes(byte[] data) throws Exception {
        byte[] encrpted = codec.encrypt(data, key);
