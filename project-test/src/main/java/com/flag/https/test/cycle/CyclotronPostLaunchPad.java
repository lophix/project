...
public class CyclotronPostLaunchPad {
    ...
    private static final ScheduledExecutorService SCHEDULED_EXECUTOR_SERVICE = Executors.newScheduledThreadPool(THREAD_NUM);
...
//                    }
                    if (System.currentTimeMillis() - start > 1000) {
                        LOGGER.info("received message is {},time {}", resEntity.getContentLength()/*EntityUtils.toString(resEntity, "utf8")*/,System.currentTimeMillis() - start);
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
